-- ============================================================
-- H4CKR — Script d'initialisation de la base de données
...
CREATE TABLE IF NOT EXISTS levels (
    ...
    solution_hash VARCHAR NOT NULL,
...
CREATE TABLE IF NOT EXISTS hints (
    ...
    malus INTEGER NOT NULL DEFAULT 10
);
...
(8, 1, 'Les métadonnées EXIF sont des informations cachées dans les fichiers image.', 10),
(8, 2, 'Utilise exiftool, Jeffrey''s Exif Viewer ou exifdata.com pour lire les métadonnées.', 20),
(8, 3, 'Cherche spécifiquement le champ "Author" ou "Artist" dans les métadonnées.', 30),

-- Niveau 9 : EXIF GPS
(9, 1, 'Les coordonnées GPS sont dans les champs "GPS Latitude" des métadonnées EXIF.', 10),
...
(10, 1, 'Les headers HTTP custom commencent par X-. Ils transportent des métadonnées supplémentaires.', 10),
...
(11, 1, 'Les codes 301, 302, 307 sont des redirections HTTP avec un header "Location".', 10),
(11, 2, 'Utilise curl -L pour suivre automatiquement les redirections.', 20),
(11, 3, 'Le nœud final contient un underscore et est en majuscules.', 30),

-- Niveau 12 : Regex avancée
(12, 1, 'Les lookaheads (?=...) vérifient une condition sans consommer de caractères.', 10),
...
-- Niveau 13 : Binaire
(13, 1, 'Chaque groupe de 8 bits représente un caractère ASCII. Convertis d''abord en décimal.', 10),
(13, 2, 'En Python : chr(int("01000011", 2)) → C. Applique ça à chaque octet.', 20),
(13, 3, 'Le résultat est 10 caractères, contient un underscore, lié à la cybersécurité.', 30),

-- Niveau 14 : Double Base64
(14, 1, 'Le message a été encodé en Base64 deux fois. Tu dois décoder deux fois de suite.', 10),
(14, 2, 'Première décode : T01FR0FfR0FURT0= → un autre string Base64. Décode ce résultat.', 20),
(14, 3, 'Le résultat final contient un underscore et est lié au concept de porte d''accès.', 30),

-- Niveau 15 : Final
(15, 1, 'Lis attentivement le JSON : il contient deux clés. Concatène leurs valeurs avec _.', 10),
